      ...
      dir ('./appeals-deployment/ansible') {
        if (env.APP_ENV == 'prod') {
          APP_VERSION = sh (
            ...
            // 5. get the last line
            script: "git ls-remote --tags https://${env.GIT_CREDENTIAL}@github.com/department-of-veterans-affairs/caseflow-feedback.git | awk '{print \$2}' | grep -E 'manual|stable' | sort -t/ -nk4 | awk -F\"/\" '{print \$0}' | tail -n 1",
            returnStdout: true
            ).trim()
        }
